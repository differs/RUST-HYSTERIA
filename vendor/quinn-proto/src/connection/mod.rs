...
impl AdaptiveGsoConfig {
    fn from_env() -> Self {
        Self::from_values(
            env::var("HY_RS_ADAPTIVE_GSO_ON_LOSS").ok(),
            env::var("HY_RS_ADAPTIVE_GSO_PACKET_THRESHOLD").ok(),
            env::var("HY_RS_ADAPTIVE_GSO_BYTES_THRESHOLD").ok(),
            env::var("HY_RS_ADAPTIVE_GSO_PERSISTENT_ONLY").ok(),
        )
    }

    fn from_values(
        enabled: Option<String>,
        packet_threshold: Option<String>,
        bytes_threshold: Option<String>,
        persistent_only: Option<String>,
    ) -> Self {
        Self {
            enabled: parse_bool_env_value(enabled).unwrap_or(true),
            packet_threshold: parse_u64_env_value(packet_threshold).unwrap_or(2).max(1),
            bytes_threshold: parse_u64_env_value(bytes_threshold),
            persistent_only: parse_bool_env_value(persistent_only).unwrap_or(true),
        }
    }
}

fn adaptive_gso_config() -> &'static AdaptiveGsoConfig {
    static CONFIG: OnceLock<AdaptiveGsoConfig> = OnceLock::new();
    CONFIG.get_or_init(AdaptiveGsoConfig::from_env)
}

fn parse_bool_env_value(value: Option<String>) -> Option<bool> {
...
fn parse_u64_env_value(value: Option<String>) -> Option<u64> {
    value?.trim().parse().ok()
}
...
impl Connection {
    ...
    pub fn poll_transmit(
        ...
        let max_datagrams = match self.config.enable_segmentation_offload
            && !self.path.disable_segmentation_offload
        {
            false => 1,
            true => max_datagrams,
        };
            ...
            if !coalesce || buf_capacity - buf_end < MIN_PACKET_SPACE + tag_len {
                ...
                if ack_eliciting && self.spaces[space_id].loss_probes == 0 {
                    ...
                    if let Some(delay) = self.path.pacing.delay(
                        ...
                        controller_metrics.pacing_behavior,
                        now,
                    ) {
    ...
    fn on_ack_received(
        ...
        for packet in newly_acked.elts() {
            if let Some(info) = self.spaces[space].take(packet) {
                ...
                if let Some(acked) = self.on_packet_acked(now, packet, info) {
        ...
        let (_lost_ack_eliciting_packets, lost_packets) =
            self.detect_lost_packets(now, space, true);
        self.path.congestion.on_ack_event(
            now,
            &crate::congestion::AckEvent {
                prior_in_flight,
                acked_packets: &acked_packets,
                lost_packets: &lost_packets,
                app_limited: self.app_limited,
                largest_packet_num_acked: acked_packets.last().map(|packet| packet.packet_number),
                rtt: &self.path.rtt,
            },
        );
    ...
    fn on_packet_acked(
        ...
        packet: u64,
        info: SentPacket,
    ) -> Option<crate::congestion::AckedPacketInfo> {
        let acked = (info.size != 0).then_some(crate::congestion::AckedPacketInfo {
            packet_number: packet,
            bytes_acked: info.size.into(),
        });
    ...
    fn on_loss_detection_timeout(&mut self, now: Instant) {
        if let Some((_, pn_space)) = self.loss_time_and_space() {
            ...
            let _ = self.detect_lost_packets(now, pn_space, false);
            self.set_loss_detection_timer(now);
            return;
        }
    ...
    fn detect_lost_packets(
        ...
        pn_space: SpaceId,
        due_to_ack: bool,
    ) -> (u64, Vec<crate::congestion::LostPacketInfo>) {
        let mut lost_packets = Vec::<u64>::new();
        let mut lost_packet_infos = Vec::<crate::congestion::LostPacketInfo>::new();
        ...
        let mut lost_ack_eliciting_packets = 0u64;
        let mut lost_ack_eliciting_packets_by_packet_threshold = 0u64;
        ...
        for (&packet, info) in space.sent_packets.range(0..largest_acked_packet) {
            ...
            let lost_by_time_threshold = info.time_sent <= lost_send_time;
            let lost_by_packet_threshold = largest_acked_packet >= packet + packet_threshold;
            if lost_by_time_threshold || lost_by_packet_threshold {
                if Some(packet) == in_flight_mtu_probe {
                    ...
                    lost_packet_infos.push(crate::congestion::LostPacketInfo {
                        packet_number: packet,
                        bytes_lost: info.size as u64,
                    });
                    size_of_lost_packets += info.size as u64;
                    if info.ack_eliciting {
                        lost_ack_eliciting_packets = lost_ack_eliciting_packets.saturating_add(1);
                        if lost_by_packet_threshold {
                            lost_ack_eliciting_packets_by_packet_threshold =
                                lost_ack_eliciting_packets_by_packet_threshold.saturating_add(1);
                        }
                    }
        ...
        if let Some(largest_lost) = lost_packets.last().cloned() {
            ...
            let adaptive_gso = adaptive_gso_config();
            if adaptive_gso.enabled
                && due_to_ack
                && lost_ack_eliciting_packets_by_packet_threshold >= adaptive_gso.packet_threshold
                && adaptive_gso
                    .bytes_threshold
                    .is_none_or(|threshold| size_of_lost_packets >= threshold)
                && (!adaptive_gso.persistent_only || in_persistent_congestion)
            {
                self.path.disable_segmentation_offload = true;
            }
        ...
        if due_to_ack {
            (lost_ack_eliciting_packets, lost_packet_infos)
        } else {
            (0, lost_packet_infos)
        }
    }
...
mod tests {
    ...
    #[test]
    fn adaptive_gso_config_uses_expected_defaults() {
        assert_eq!(
            AdaptiveGsoConfig::from_values(None, None, None, None),
            AdaptiveGsoConfig {
                enabled: true,
                packet_threshold: 2,
    ...
    #[test]
    fn adaptive_gso_config_parses_overrides() {
        let config = AdaptiveGsoConfig::from_values(
            Some("off".into()),
            Some("5".into()),
            Some("4096".into()),
            Some("yes".into()),
    ...
    #[test]
    fn adaptive_gso_config_ignores_invalid_overrides() {
        let config = AdaptiveGsoConfig::from_values(
            Some("maybe".into()),
            Some("garbage".into()),
            Some("".into()),
