...
impl StreamsState {
    ...
    pub(super) fn add_read_credits(&mut self, credits: u64) -> ShouldTransmit {
        ...
        ShouldTransmit(diff >= flow_control_update_threshold(self.receive_window))
    }
...
fn flow_control_update_threshold(window: u64) -> u64 {
    let tuning = flow_control_tuning();
    (window / tuning.divisor.max(1))
        .max(tuning.min_threshold)
        .min(tuning.max_threshold)
        .min(window.max(1))
}

#[derive(Clone, Copy)]
struct FlowControlTuning {
    divisor: u64,
    min_threshold: u64,
    max_threshold: u64,
}

fn flow_control_tuning() -> &'static FlowControlTuning {
    static TUNING: OnceLock<FlowControlTuning> = OnceLock::new();
    TUNING.get_or_init(|| FlowControlTuning {
        divisor: env_u64("HY_RS_FLOW_CONTROL_DIVISOR").unwrap_or(32),
        min_threshold: env_u64("HY_RS_FLOW_CONTROL_MIN_THRESHOLD").unwrap_or(64 * 1024),
        max_threshold: env_u64("HY_RS_FLOW_CONTROL_MAX_THRESHOLD").unwrap_or(2 * 1024 * 1024),
    })
}
